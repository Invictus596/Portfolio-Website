...
window.onerror = (msg, url, line, col, err) => {
  ...
  document.body.innerHTML =
    `<div style="background:#111;color:red;padding:2rem;font-family:monospace">` +
    `<h1>Error</h1><pre>${msg}</pre>` +
    (err?.stack ? `<pre style="color:#888;font-size:12px">${err.stack}</pre>` : "") +
    `</div>`;
  return true;
};
